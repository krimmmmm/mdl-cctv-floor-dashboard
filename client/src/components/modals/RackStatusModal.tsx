...
const RackStatusModal: React.FC<RackStatusModalProps> = ({
  ...
  const [equipmentName, setEquipmentName] = useState(rack?.name || "");
  const { user } = useAuth();
  const savedUser =
  ...
  const handleSaveName = () => {
    ...
    updateRackField(rack.id, "name", nextName);
    ...
    <>
      <div style={styles.overlay} onClick={onClose}>
        <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
          <h2 style={styles.title}>{rack.name || "Rack"}</h2>
