...
const FloorPlanCanvas: React.FC = () => {
  ...
  useEffect(() => {
    setCameraCountInput((cameras || []).length);
  }, [cameras]);
  ...
  const handleCameraCountChange = (value: number) => {
    const safeValue = Math.max(0, value);

    setCameraCountInput(safeValue);

    if (typeof setCameraCount === 'function') {
      setCameraCount(safeValue);
    }
  };
  ...
  const handleCabinetCountChange = (value: number) => {
    ...
    if (typeof setCabinetCount === 'function') {
      setCabinetCount(safeValue);
    } else {
      alert('ยังไม่ได้สร้างระบบ Cabinet Count ใน FloorPlanContext');
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-30 bg-white rounded-2xl shadow-2xl border border-gray-200 p-5 w-72">
        ...
        <div className="mb-5">
          <div className="flex justify-between mb-2">
            <span className="font-semibold text-gray-700">Cameras</span>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleCameraCountChange(cameraCountInput - 1);
              }}
        ...
        <div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={(e) => {
                e.stopPropagation();
                handleCabinetCountChange(cabinetCountInput - 1);
              }}
              className="w-10 h-10 rounded-lg bg-red-500 hover:bg-red-600 text-white text-xl font-bold"
            >
              -
      ...
      >
        ...
        {(fiberRoutes || []).map((route) => (
          <g
            ...
            onClick={(e) => {
              e.stopPropagation();
              handleFiberClick(route.id);
            }}
            style={{
              cursor: canvasMode === 'normal' ? 'pointer' : 'crosshair',
            }}
          >
            <path
              d={(route.points || []).reduce(
                (acc, pt, i) =>
                  i === 0 ? `M ${pt.x} ${pt.y}` : `${acc} L ${pt.x} ${pt.y}`,
                ''
              )}
        ...
        {(cameras || []).map((camera) => {
          const isSelected =
            selectedItem?.type === 'camera' && selectedItem.id === camera.id;
        ...
        {(racks || []).map((rack) => {
          const isSelected =
            selectedItem?.type === 'rack' && selectedItem.id === rack.id;
        ...
        {(cabinets || []).map((cabinet) => {
          const isSelected =
            selectedItem?.type === 'cabinet' && selectedItem.id === cabinet.id;
      ...
      {selectedRack && !isMovingEquipment && !showPositionModal && (
        <RackStatusModal
          rack={selectedRack}
          isOpen={true}
          onClose={() => setSelectedItem(null)}
      ...
      {selectedCabinet && !isMovingEquipment && !showPositionModal && (
        <CabinetStatusModal
      ...
      <div className="absolute bottom-4 right-4 bg-white rounded-lg shadow-md px-4 py-2 text-sm text-gray-600">
        {canvasMode === 'draw_fiber'
          ? `Drawing Fiber · ${(drawingPoints || []).length} point${
