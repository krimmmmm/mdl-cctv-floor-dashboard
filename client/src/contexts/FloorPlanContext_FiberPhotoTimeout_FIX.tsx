...
const toDbFiberRouteChanges = (changes: any) => {
  const payload: any = {};

  if ("points" in changes && Array.isArray(changes.points) && changes.points.length > 0) {
    payload.points = changes.points;
  }

  if ("color" in changes) payload.color = changes.color || "#ef4444";
  if ("label" in changes || "name" in changes) {
    payload.label = changes.label || changes.name || "Fiber Route";
  }
  if ("status" in changes) payload.status = changes.status || "active";
  if ("progress" in changes) payload.progress = Number(changes.progress || 0);
  if ("progressDirection" in changes) {
    payload.progress_direction = changes.progressDirection || "start";
  }
  if ("photo1" in changes) payload.photo1 = changes.photo1 || "";
  if ("photo2" in changes) payload.photo2 = changes.photo2 || "";
  if ("photo3" in changes) payload.photo3 = changes.photo3 || "";
  if ("photo4" in changes) payload.photo4 = changes.photo4 || "";
...
export const FloorPlanProvider = ({
  ...
  const updateFiberRoute = async (id: string, changes: any) => {
    ...
    */
    ...
    /*
      IMPORTANT:
      Update ONLY changed DB columns.
      This prevents progress/name edits from repeatedly sending large
      Base64 photo fields and reduces statement timeout risk.
    */
    const dbPayload = toDbFiberRouteChanges({
      ...changes,
      ...(hasValidIncomingPoints ? { points: preservedPoints } : {}),
    });
